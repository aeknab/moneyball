...
def display_predictions_overview():
    ...
    # Add "--" as the default option in the dropdown
    matchday_options = ['--'] + list(buli_df_2023['Matchday'].unique()[::-1])
    
    # Dropdown to select matchday with "--" as the default
    selected_matchday = st.selectbox("Select Matchday", options=matchday_options, key="matchday_select_predictions_overview")

    # Store the selected matchday in session state
    if selected_matchday != '--':
        st.session_state.selected_matchday = selected_matchday
    else:
        st.write("Please select a matchday to view matches.")
        return
